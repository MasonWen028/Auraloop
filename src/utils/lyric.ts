...
export const parseLrcData = (lrcData: LyricLine[]): LyricType[] => {
  ...
  const lrcList = lrcData
    .map((line) => {
      ...
      // 排除内容
      if (!content || getExcludeKeywords().some((keyword: any) => content.includes(keyword))) {
        return null;
      }
      return {
        time,
...
export const parseYrcData = (yrcData: LyricLine[]): LyricType[] => {
  ...
  const yrcList = yrcData
    .map((line) => {
      ...
      const contents = words.map((word: any) => {
        return {
      ...
      const contentStr = contents
        .map((word: any) => word.content + (word.endsWithSpace ? " " : ""))
        .join("");
      // 排除内容
      if (!contentStr || getExcludeKeywords().some((keyword: any) => contentStr.includes(keyword))) {
        return null;
      }
      return {
        time,
